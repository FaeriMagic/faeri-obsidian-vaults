...
<%* await tp.file.create_new(tp.file.find_tfile("MyTemplate"), "MyFilename") %> // File creation and open created note <%* await tp.file.create_new("MyFileContent", "MyFilename", true) %>
...
← Go to [[Templater - Documentation (2.3. tp.date)|previous page]] | Go to [[Templater - Documentation (2.5. tp.frontmatter)|next page]] →